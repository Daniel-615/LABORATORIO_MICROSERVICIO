...
from src.utils.OracleHelper import OracleHelper
...
class Main:
    def __init__(self):
        ...
        self.app_initializer = AppInitializer(self.getApp(), self.db, self.models)

        with self.app.app_context():
                self.db.create_all() 
                oracle_helper = OracleHelper(self.db)
                oracle_helper.create_auto_increment("FACTURA_DETALLE")
                oracle_helper.create_auto_increment("PRODUCTO")
